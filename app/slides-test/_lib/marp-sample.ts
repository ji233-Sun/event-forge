...
> = {
  slate: {
    light: {
      ...
      cardBorder: "rgba(71,85,105,0.22)",
    },
    dark: {
      slideBg: "#0f172a",
      slideText: "#e2e8f0",
  ...
  zinc: {
    ...
    dark: {
      ...
      cardBorder: "rgba(161,161,170,0.24)",
    },
  },
  neutral: {
    light: {
      ...
      cardBorder: "rgba(82,82,82,0.24)",
    },
    dark: {
      slideBg: "#171717",
      slideText: "#f5f5f5",
      slideMuted: "#a3a3a3",
      cardBg: "rgba(23,23,23,0.62)",
      cardBorder: "rgba(163,163,163,0.24)",
    },
  },
};
...
function resolvePalette(values: TemplateValues): Palette {
...
function buildFontImport(heading: TemplateValues["headingFont"], body: TemplateValues["bodyFont"]): string {
  ...
  return `@import url('https://fonts.googleapis.com/css2?${families.map((item) => `family=${item}`).join("&")}&display=swap');`;
}
...
function buildDynamicStyle(values: TemplateValues): { style: string; palette: Palette } {
