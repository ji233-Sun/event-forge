...
import { SYSTEM_PROMPT, parseGeneratedCustomTypeResult } from '../generate/route'

const ITERATION_SYSTEM_PROMPT = `${SYSTEM_PROMPT}

ITERATION MODE — MANDATORY:
- You are revising an existing custom survey question type, not starting from scratch.
- Use the original prompt as the product brief and the feedback as the exact change request.
- Preserve working behavior unless the feedback explicitly asks to remove or replace it.
...
function isValidCurrentResult(value: unknown): value is GenerateCustomTypeResult {
  return (
    isRecord(value)
    && typeof value.suggestedName === 'string'
    && typeof value.formCode === 'string'
    && typeof value.displayCode === 'string'
    && isRecord(value.answerSchema)
  )
}
...
export async function POST(request: Request) {
  ...
  try {
    const result = await generateText({
      ...
      prompt: [
        'Revise this custom survey question type based on the current implementation.',
        `Original request:\n${originalPrompt.trim()}`,
        `Feedback to apply:\n${feedback.trim()}`,
        `Current result JSON:\n${JSON.stringify(currentResult, null, 2)}`,
        'Return the full revised JSON object with suggestedName, formCode, displayCode, and answerSchema.',
      ].join('\n\n'),
    })
